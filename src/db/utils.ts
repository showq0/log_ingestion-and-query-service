import { SQL, eq, gte, lt, lte, ilike, sql } from "drizzle-orm";
...
export function createLogConditions(
  ...
  if (parameter.service) {
    conditions.push(eq(logs.service, parameter.service));
  }
  //Exact level match
  if (parameter.level) {
    conditions.push(eq(logs.level, parameter.level));
  }
  //Inclusive start of the time range
  if (parameter.since) {
    conditions.push(gte(logs.timestamp, parameter.since));
  }
  //Exclusive end of the time range
  if (parameter.until) {
    conditions.push(lt(logs.timestamp, parameter.until));
  }
  //Case-insensitive substring match ilike
  if (parameter.q) {
    conditions.push(ilike(logs.message, `%${parameter.q}%`));
  }
  if (parameter.cursor) {
